...
vignette: >
  %\VignetteIndexEntry{bain_introduction}
...
probability including the unconstrained hypothesis. 
Note that, the posterior model probabilities are computed from
...
a named vector object. This option uses `bain.default()`, so the user must manually extract the required information. See van Lissa et al. (unpublished) for further elaborations. First, we fit a structural equation model using `lavaan`:
